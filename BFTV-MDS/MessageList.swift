...
class MessageList: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Mensagens" //Set Navigation Bar title to "Noticias"
    }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return UtilVariables.mensagens.count //Set the number of elementes equal to number of news in UtilVariables class.
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "MessagePostSegue"{ //If transition to the next screen equals to "NewsPostSegue", or if the next screen is the post screen.
            let destination = segue.destination as? MessagePost //Instatiate the destination class.
